...
class AnalystContext:
    ...
    vol_forecast: Any | None = None  # quant.forecast.VolForecast | None (caller-supplied)
    macro_regime: Any | None = None  # quant.forecast.MacroRegimeRead | None (caller-supplied)
...
def render_context(ctx: AnalystContext) -> str:
    ...
    if ctx.macro_nowcast is not None:
        with contextlib.suppress(Exception):  # render is best-effort
    ...
    if ctx.vol_forecast is not None:
        with contextlib.suppress(Exception):  # render is best-effort
            from quant.forecast.vol import render_vol_forecast

            lines.append(render_vol_forecast(ctx.vol_forecast))

    if ctx.macro_regime is not None:
        with contextlib.suppress(Exception):  # render is best-effort
            from quant.forecast.regime import render_macro_regime

            lines.append(render_macro_regime(ctx.macro_regime))
