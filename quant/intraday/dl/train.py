...
def train_model(
    ...
    # Enable deterministic algorithms for this training run, then restore the prior global
    # state in `finally` so train_model leaves no process-wide side-effect (the setting is a
    # process-global flag). CPU LSTM ops are deterministic under it.
