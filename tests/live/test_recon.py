...
def _bar_fetcher_for(prices: dict[tuple[str, date], float]) -> Callable[[str, date], float | None]:
    """Return a fake bar fetcher keyed on (symbol, signal_date)."""

    def fetch(symbol: str, signal_date: date) -> float | None:
        return prices.get((symbol, signal_date))
...
def test_reconcile_clean_one_to_one_buy() -> None:
    ...
    orders = [_order()]
    bars = _bar_fetcher_for({("SPY", date(2026, 5, 26)): 499.87})  # prior trading day
...
def test_reconcile_clean_one_to_one_sell_signed_correctly() -> None:
    ...
    bars = _bar_fetcher_for({("SPY", date(2026, 5, 26)): 499.87})
...
def test_reconcile_partial_fill_computes_slippage_on_filled_portion() -> None:
    ...
    orders = [
        _order(submitted_qty=100, filled_qty=60, filled_avg_price=500.12, status="partially_filled")
    ]
    bars = _bar_fetcher_for({("SPY", date(2026, 5, 26)): 499.87})
...
def test_report_aggregate_by_strategy() -> None:
    ...
    orders = [
        _order(
            coid="trend-20260526-SPY-a",
            symbol="SPY",
            submitted_qty=100,
            filled_qty=100,
            filled_avg_price=500.12,
        ),
        _order(
            coid="trend-20260526-DBC-b",
            symbol="DBC",
            submitted_qty=50,
            filled_qty=50,
            filled_avg_price=25.05,
        ),
        _order(
            coid="momentum-20260526-EFA-c",
            symbol="EFA",
            submitted_qty=80,
            filled_qty=80,
            filled_avg_price=80.20,
        ),
    ]
    bars = _bar_fetcher_for(
        {
            ("SPY", date(2026, 5, 26)): 499.87,
            ("DBC", date(2026, 5, 26)): 25.00,
            ("EFA", date(2026, 5, 26)): 80.00,
        }
    )
...
def test_report_aggregate_by_symbol() -> None:
    ...
    orders = [
        ...
        _order(
            coid="trend-20260526-SPY-b",
            symbol="SPY",
            submitted_qty=50,
            filled_qty=50,
            filled_avg_price=500.50,
        ),
    ]
    bars = _bar_fetcher_for({("SPY", date(2026, 5, 26)): 499.87})
