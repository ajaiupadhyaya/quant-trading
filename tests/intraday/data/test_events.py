...
def test_quotebar_mid_and_spread():
    q = QuoteBar(
        ts=_ts("2023-06-01T13:30:00"), symbol="AAPL", bid=100.0, ask=100.04, bid_size=5, ask_size=7
    )
...
def test_event_sort_key_orders_by_ts_then_type_then_symbol():
    ...
    bar = Bar(
        ts=t, symbol="AAPL", open=1, high=2, low=1, close=2, volume=10, vwap=1.5, trade_count=3
    )
