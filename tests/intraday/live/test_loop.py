...
def test_feed_error_skips_tick(tmp_path):
    ...
    broker, ledger = _Broker(), SleeveLedger()
    deps = _deps(tmp_path, broker, _BadFeed(),
                 _Strat([Order("QQQ", Side.BUY, 10)]), ledger,
                 now=datetime(2026, 6, 8, 15, 0, tzinfo=UTC),
                 close=datetime(2026, 6, 8, 20, 0, tzinfo=UTC))
...
def test_trade_budget_blocks_new_opens(tmp_path):
    ...
    deps = TickDeps(
        data_dir=tmp_path, config=SleeveConfig(max_round_trips=1), broker=broker,
        feed=_Feed([_qb("QQQ", 100.0)]), strategy=_Strat([Order("QQQ", Side.BUY, 10)]),
        ledger=ledger, now=datetime(2026, 6, 8, 15, 0, tzinfo=UTC),
...
def test_strategy_exit_executes_when_sleeve_full(tmp_path):
    ...
    deps = TickDeps(
        data_dir=tmp_path, config=SleeveConfig(notional_cap_pct=0.0001, notional_cap_abs=10.0),
        broker=broker, feed=_Feed([_qb("QQQ", 100.0)]),
...
def test_short_flatten_buys_to_cover(tmp_path):
    ...
    deps = _deps(tmp_path, broker, _Feed([_qb("QQQ", 100.0)]),
                 _Strat([Order("QQQ", Side.SELL, 10)]), ledger,
