...
def _deps(tmp_path, broker, feed, strat, ledger, *, tick_index, mgr, now=None):
    return TickDeps(
        data_dir=tmp_path,
        config=SleeveConfig(
            notional_cap_pct=1.0, notional_cap_abs=1e9, per_trade_cap=1e9, mean_reversion_lookback=5
        ),
        broker=broker,
        feed=feed,
        strategy=strat,
...
def test_entry_is_worked_over_ticks_not_dumped(tmp_path):
    ...
    silent = _Strat([])
    run_tick(
        _deps(tmp_path, broker, _Feed([_qb("QQQ", 100.0)]), silent, ledger, tick_index=1, mgr=mgr)
    )
    run_tick(
        _deps(tmp_path, broker, _Feed([_qb("QQQ", 100.0)]), silent, ledger, tick_index=2, mgr=mgr)
    )
