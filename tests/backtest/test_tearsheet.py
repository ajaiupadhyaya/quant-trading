...
# Module-scoped: the four populated-walkforward tests all reuse the same WF result,
# so we compute it once and avoid a 4x repeat of the 7-year backtest cycle.
@pytest.fixture(scope="module")
def populated_wf() -> WalkforwardResult:
    bars = synthetic_bars(["AAA", "BBB"], date(2010, 1, 1), date(2017, 1, 1), seed=0)
...
def test_tearsheet_writes_three_files(tmp_path: Path, populated_wf: WalkforwardResult) -> None:
...
def test_tearsheet_walkforward_parquet_matches_oos_curve(
    ...
    assert len(on_disk) == len(populated_wf.oos_equity_curve)
