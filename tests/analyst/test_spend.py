...
def test_cost_usd_cache_multipliers_and_unknown_model() -> None:
    ...
    assert abs(cost_usd("claude-opus-4-8", input_tokens=0, output_tokens=0,
                        cache_read_tokens=1_000_000) - 0.5) < 1e-9
    assert abs(cost_usd("claude-opus-4-8", input_tokens=0, output_tokens=0,
                        cache_write_tokens=1_000_000) - 6.25) < 1e-9
...
def test_record_spend_writes_and_computes_cost(tmp_path) -> None:
    rec = record_spend(
        call_site="digest", model="claude-haiku-4-5",
        usage=_usage(2000, 500), data_dir=tmp_path, now=_NOW,
    )
...
def test_record_spend_failopen_on_missing_usage(tmp_path) -> None:
    assert record_spend(
        call_site="watch", model="claude-opus-4-8", usage=None, data_dir=tmp_path
    ) is None
...
def test_load_records_skips_corrupt_lines(tmp_path) -> None:
    record_spend(call_site="watch", model="claude-opus-4-8", usage=_usage(10, 10),
                 data_dir=tmp_path, now=_NOW)
...
def test_summarize_aggregates(tmp_path) -> None:
    record_spend(call_site="digest", model="claude-haiku-4-5", usage=_usage(1000, 0),
                 data_dir=tmp_path, now=datetime(2026, 6, 8, 12, tzinfo=UTC))
    record_spend(call_site="watch", model="claude-opus-4-8", usage=_usage(1000, 0),
                 data_dir=tmp_path, now=_NOW)
...
def test_over_daily_budget(tmp_path) -> None:
    record_spend(call_site="watch", model="claude-opus-4-8", usage=_usage(1_000_000, 0),
                 data_dir=tmp_path, now=_NOW)  # $5.00 on 2026-06-09
    rows = load_records(tmp_path)
