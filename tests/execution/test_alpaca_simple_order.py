...
def test_submit_simple_order_market_uses_supplied_coid():
    ...
    coid = c.submit_simple_order(
        symbol="QQQ", side="buy", qty=5, client_order_id="sleeve:QQQ:123:0"
    )
...
def test_submit_simple_order_dry_run_skips_broker():
    c = _client_with_fake_trading()
    coid = c.submit_simple_order(
        symbol="IWM", side="sell", qty=3, client_order_id="sleeve:IWM:9:1", dry_run=True
    )
